...
for dr in defect_rates:
    plt.boxplot(
        ...
        labels=[15, 21, 27],
        patch_artist=True,
        boxprops=dict(facecolor=color[dr]),
        medianprops=dict(color='black'),
        flierprops=dict(marker='.', markerfacecolor=color[dr]),
    )
...
plt.yscale('log')
legend_handles = [
    Patch(facecolor=color[dr], label=f'Defect rate={dr}')
    for dr in defect_rates
] + [Line2D([0], [0], label='Perfect')]

plt.legend(handles=legend_handles)

plt.title('LER vs Distance')
plt.xlabel('Distance')
plt.ylabel('LER')
plt.show()